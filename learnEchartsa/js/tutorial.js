window.onload = function() {
	...
	// 柱状图
	myEcharts.drawBarChart();

	// 南丁格尔图
	myEcharts.drawPieChart();

	// 散点图
	myEcharts.drawScatterChart();

	// 人口散点图
	myEcharts.drawScaPeopleLifeChart();

	// 百度地图 & echart geo组件展示空气质量
	myEcharts.drawMapAirQualityChart();

	// 几个与收入展示图
	myEcharts.drawScatPriceAndEarnChart();

	// 安斯克姆四重奏
	myEcharts.drawScatAnscombeChart();

	// pm2.5指数
	myEcharts.drawScaPMAQI();
};
...
var myEcharts = {
			...
			if (params.componentType === 'markPoint') {
		        ...
		        if (params.seriesType === 'graph') {
		            if (params.dataType === 'edge') {
		                ...
		                alert('点击到了 graph 的 edge（边）上。');
		            }
		            else {
		                // 点击到了 graph 的 node（节点）上。
		                alert('点击到了 graph 的 node（节点）上。');
		            }
	...
	drawScatterChart: function() {
		var scatterChart = echarts.init(document.getElementById('line'));
	...
	drawScaPeopleLifeChart: function() {
		var plChart = echarts.init(document.getElementById('peoplelife')),
		data = [
	...
	drawMapAirQualityChart: function() {
		var bAairqChart = echarts.init(document.getElementById('bAirQuality')),
		airqChart = echarts.init(document.getElementById('airQuality'));
		...
		bAairqChart.setOption(option);
		airqChart.setOption(option1);
	},

	drawScatPriceAndEarnChart: function() {
		var myChart = echarts.init(document.getElementById('priceAEarn'));
	...
	drawScatAnscombeChart: function() {
		var dataAll = [
		    [
		        [10.0, 8.04],
		        [8.0, 6.95],
		...
		var myChart = echarts.init(document.getElementById('anscombe'));
		var markLineOpt = {
			animation: false,
			label: {
				normal: {
	...
	drawScaPMAQI: function() {
		...
		var myChart = echarts.init(document.getElementById('aqiColor'));
		var option = {
			backgroundColor: '#404a59',
